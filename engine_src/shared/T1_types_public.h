#ifndef T1_PUBLIC_TYPES_H
#define T1_PUBLIC_TYPES_H

#include "T1_stdint.h"
#include "T1_types_public_gpucpu.h"

#define T1_ID_NONE 0xFFFFFFFF
// retired objects are unaffected by animations
#define T1_ID_RETIRED_BUT_NOT_DELETED 20501

#define T1_TOUCH_ID_NONE 0xFFFFFFFF

#define T1_ANIM_NO_EFFECT 0x0FFFFFFF
#define T1_ANIM_HIT_EVERYTHING 0x0AAAAAAA

// Basic quads and cubes are predefined, they can be used without registering
// an .obj file.
#define T1_BASIC_QUAD_MESH_ID 0
#define T1_BASIC_CUBE_MESH_ID 1

typedef u16 T1Tex;

typedef enum : u16 {
    T1_IO_KEYBOARD_LEFTARROW = 0,
    T1_IO_KEYBOARD_RIGHTARROW = 1,
    T1_IO_KEYBOARD_UPARROW = 2,
    T1_IO_KEYBOARD_DOWNARROW = 3,
    T1_IO_KEYBOARD_ENTER = 4,
    T1_IO_KEYBOARD_ESCAPE = 5,
    T1_IO_KEYBOARD_TAB = 6,
    T1_IO_KEYBOARD_SPACEBAR = 7,
    T1_IO_KEYBOARD_BACKSPACE = 8,
    T1_IO_KEYBOARD_YENSIGN = 9,
    T1_IO_KEYBOARD_KANABUTTON = 10,
    T1_IO_KEYBOARD_ROMAJIBUTTON = 11,
    T1_IO_KEYBOARD_ALT = 12,
    T1_IO_KEYBOARD_WINKEY = 13,
    T1_IO_KEYBOARD_CAPSLOCK = 14,
    T1_IO_KEYBOARD_CONTROL = 15,
    T1_IO_KEYBOARD_SHIFT = 16,
    T1_IO_KEYBOARD_UNKNOWNBUTTON = 17,
    T1_IO_KEYBOARD_EXCLAM = 33,
    T1_IO_KEYBOARD_DOUBLEQUOTE = 34,
    T1_IO_KEYBOARD_NUMBERSIGN = 35,
    T1_IO_KEYBOARD_DOLLARSIGN = 36,
    T1_IO_KEYBOARD_PERCENTSIGN = 37,
    T1_IO_KEYBOARD_AMPERSAND = 38,
    T1_IO_KEYBOARD_APOSTROPHE = 39,
    T1_IO_KEYBOARD_LEFTPAREN = 40,
    T1_IO_KEYBOARD_RIGHTPAREN = 41,
    T1_IO_KEYBOARD_ASTERISK = 42,
    T1_IO_KEYBOARD_PLUS = 43,
    T1_IO_KEYBOARD_COMMA = 44,
    T1_IO_KEYBOARD_MINUS = 45,
    T1_IO_KEYBOARD_FULLSTOP = 46,
    T1_IO_KEYBOARD_BACKSLASH = 47,
    T1_IO_KEYBOARD_0 = 48,
    T1_IO_KEYBOARD_1 = 49,
    T1_IO_KEYBOARD_2 = 50,
    T1_IO_KEYBOARD_3 = 51,
    T1_IO_KEYBOARD_4 = 52,
    T1_IO_KEYBOARD_5 = 53,
    T1_IO_KEYBOARD_6 = 54,
    T1_IO_KEYBOARD_7 = 55,
    T1_IO_KEYBOARD_8 = 56,
    T1_IO_KEYBOARD_9 = 57,
    T1_IO_KEYBOARD_COLON = 58,
    T1_IO_KEYBOARD_SEMICOLON = 59,
    T1_IO_KEYBOARD_LESSTHAN = 60,
    T1_IO_KEYBOARD_EQUALS = 61,
    T1_IO_KEYBOARD_GREATERTHAN = 62,
    T1_IO_KEYBOARD_QMARK = 63,
    T1_IO_KEYBOARD_AT = 64,
    T1_IO_KEYBOARD_A = 65,
    T1_IO_KEYBOARD_B = 66,
    T1_IO_KEYBOARD_C = 67,
    T1_IO_KEYBOARD_D = 68,
    T1_IO_KEYBOARD_E = 69,
    T1_IO_KEYBOARD_F = 70,
    T1_IO_KEYBOARD_G = 71,
    T1_IO_KEYBOARD_H = 72,
    T1_IO_KEYBOARD_I = 73,
    T1_IO_KEYBOARD_J = 74,
    T1_IO_KEYBOARD_K = 75,
    T1_IO_KEYBOARD_L = 76,
    T1_IO_KEYBOARD_M = 77,
    T1_IO_KEYBOARD_N = 78,
    T1_IO_KEYBOARD_O = 79,
    T1_IO_KEYBOARD_P = 80,
    T1_IO_KEYBOARD_Q = 81,
    T1_IO_KEYBOARD_R = 82,
    T1_IO_KEYBOARD_S = 83,
    T1_IO_KEYBOARD_T = 84,
    T1_IO_KEYBOARD_U = 85,
    T1_IO_KEYBOARD_V = 86,
    T1_IO_KEYBOARD_W = 87,
    T1_IO_KEYBOARD_X = 88,
    T1_IO_KEYBOARD_Y = 89,
    T1_IO_KEYBOARD_Z = 90,
    T1_IO_KEYBOARD_OPENSQUARE = 91,
    T1_IO_KEYBOARD_CLOSESQUARE = 93,
    T1_IO_KEYBOARD_HAT = 94,
    T1_IO_KEYBOARD_UNDERSCORE = 95,
    T1_IO_KEYBOARD_NUMPADCLEAR = 96,
    T1_IO_KEYBOARD_OPENCURLY = 123,
    T1_IO_KEYBOARD_PIPE = 124,
    T1_IO_KEYBOARD_RIGHTCURLY = 125,
    T1_IO_KEYBOARD_TILDE = 126,
    T1_IO_KEYBOARD_F1 = 136,
    T1_IO_KEYBOARD_F2 = 137,
    T1_IO_KEYBOARD_F3 = 138,
    T1_IO_KEYBOARD_F4 = 139,
    T1_IO_KEYBOARD_F5 = 140,
    T1_IO_KEYBOARD_F6 = 141,
    T1_IO_KEYBOARD_F7 = 142,
    T1_IO_KEYBOARD_F8 = 143,
    T1_IO_KEYBOARD_F9 = 144,
    T1_IO_KEYBOARD_F10 = 145,
    T1_IO_KEYBOARD_F11 = 146,
    T1_IO_KEYBOARD_F12 = 147,
    T1_IO_KEYBOARD_INSERT = 148,
    T1_IO_KEYBOARD_HOME = 149,
    T1_IO_KEYBOARD_PAGEUP = 150,
    T1_IO_KEYBOARD_END = 151,
    T1_IO_KEYBOARD_PAGEDOWN = 152,
    T1_IO_KEYBOARD_ABOVE_KEYBOARD_BOUNDS = 153,
    T1_IO_MOUSE = 154,
    T1_IO_MOUSE_LCLICK = 155,
    T1_IO_MOUSE_MCLICK = 156,
    T1_IO_MOUSE_RCLICK = 157,
    T1_IO_MOUSE_OTHERCLICK1 = 158,
    T1_IO_MOUSE_OTHERCLICK2 = 159,
    T1_IO_MOUSE_OTHERCLICK3 = 160,
    T1_IO_MOUSE_OTHERCLICK4 = 161,
    T1_IO_MOUSE_WHEEL_UP = 162,
    T1_IO_MOUSE_WHEEL_DOWN = 163,
    T1_IO_GAMEPAD_DPAD_LEFT = 164,
    T1_IO_GAMEPAD_DPAD_RIGHT = 165,
    T1_IO_GAMEPAD_DPAD_UP = 166,
    T1_IO_GAMEPAD_DPAD_DOWN = 167,
    T1_IO_GAMEPAD_LEFTSHOULDER = 168,
    T1_IO_GAMEPAD_RIGHTSHOULDER = 169,
    T1_IO_GAMEPAD_LEFTTRIGGER = 170,
    T1_IO_GAMEPAD_RIGHTTRIGGER = 171,
    T1_IO_GAMEPAD_LEFTTHUMBSTICKBUTTON = 172,
    T1_IO_GAMEPAD_RIGHTTHUMBSTICKBUTTON = 173,
    T1_IO_GAMEPAD_LEFTTHUMBSTICK = 174,
    T1_IO_GAMEPAD_RIGHTTHUMBSTICK = 175,
    T1_IO_GAMEPAD_A = 176,
    T1_IO_GAMEPAD_B = 177,
    T1_IO_GAMEPAD_X = 178,
    T1_IO_GAMEPAD_Y = 179,
    T1_IO_GAMEPAD_HOME = 180,
    T1_IO_GAMEPAD_MENU = 181,
    T1_IO_GAMEPAD_OPTIONS = 182,
    T1_IO_KEY_ABOVEBOUNDS, // must be last
} T1IOKey;

typedef struct {
    T1GPUPostProcConsts postproc_consts;
    
    u64 elapsed;
    u64 this_frame_timestamp_us;
    u64 last_resize_request_us;
    
    u32 startup_bytes_to_load;
    u32 startup_bytes_loaded;
    
    f32 last_clickray_origin[3];
    f32 last_clickray_direction[3];
    f32 last_clickray_collision[3];
    
    f32 timedelta_mult;
    f32 window_wh[2];
    f32 window_left;
    f32 window_bottom;
    
    u8 draw_mouseptr;
    u8 draw_imputed_normals;
    u8 draw_triangles;
    u8 draw_axes;
    u8 draw_fps;
    u8 draw_touch_id;
    u8 draw_scene_id;
    u8 show_profiler;
    u8 pause_profiler;
    u8 block_mouse;
    u8 block_render_view_pos_updates;
    u8 fullscreen;
    u8 clientlogic_early_startup_finished;
    u8 upcoming_fullscreen_request;
} T1Globals;

typedef struct {
    T1GPUTexQuadf32 f32s;
    T1GPUTexQuadu32 u32s;
    f32             font_height;
    u32             highlight_i;
    u32             highlight_size;
    u32             opaque_back_active;
} T1TextFontSettings;

typedef struct {
    f32 offset_xyz[3];
    u32 T1_id;
    u8 one_frame_only;
    u8 committed;
    u8 visible;
    u8 deleted;
} T1CPUTexQuad;

typedef struct {
    T1CPUTexQuad * cpu;
    T1GPUTexQuad * gpu;
} T1TexQuadRequest;

/*
TOKENIZATION
*/
typedef enum : u8 {
    T1_TOKEN_STOREMODE_DISCARD_TOKEN,    // Don't even register the token
    T1_TOKEN_STOREMODE_DISCARD_STRING,   // Register token, discard string
    T1_TOKEN_STOREMODE_FULLSTARTMIDSTOP, // Register token, save string
    T1_TOKEN_STOREMODE_MIDDLE_STRING     // Register token, save string
} T1TokenStoreMode;

/*
META TYPES aka "reflection"
*/
typedef enum : u8 {
    T1_TYPE_NOTSET,
    T1_TYPE_STRUCT,
    T1_TYPE_F32,
    T1_TYPE_U64,
    T1_TYPE_U32,
    T1_TYPE_U16,
    T1_TYPE_U8,
    T1_TYPE_U4x2,
    T1_TYPE_I64,
    T1_TYPE_I32,
    T1_TYPE_I16,
    T1_TYPE_I8,
    T1_TYPE_CHAR,
} T1MetaType;

typedef struct {
    f32 volume;
    f32 sfx_volume;
    f32 music_volume;
} T1AudioSettingsFullyPublic;

#endif // T1_PUBLIC_TYPES_H
