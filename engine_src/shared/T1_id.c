...
u32 T1_id_next_ui_element_id(void) {
    T1_log_assert(T1_id_latest_ui + 1 < T1_ID_FIRST_NONUI);
...
u32 T1_id_next_nonui_id(void) {
    ...
    T1_log_assert(T1_id_latest_nonui + 1 < T1_ID_MAX);
    return T1_id_latest_nonui++;
}

u32 T1_id_next_ui_element_touch_id(void)
{
    T1_log_assert(T1_touch_id_latest_ui >= 0);
    T1_log_assert(T1_touch_id_latest_ui < T1_ID_FIRST_NONUI_TOUCH);
...
u32 T1_id_next_nonui_touch_id(void) {
    T1_log_assert(T1_touch_id_latest_nonui >=
        T1_ID_FIRST_NONUI_TOUCH);
    T1_log_assert(T1_touch_id_latest_nonui < INT32_MAX);
