...
SIMD_VEC4F T1_triangle_normalize_vertex_vec4f(
    SIMD_VEC4F to_normalize_xyz);

void T1_triangle_normalize_zvertex_f3(
    f32 to_normalize_xyz[3]);

void T1_triangle_normalize_vertex(
    f32 * to_normalize);

SIMD_VEC4F T1_triangle_x_rotate_vec4f_known_cossin(
    SIMD_VEC4F xyz,
    f32 cos_x_angle,
    f32 sin_x_angle);
